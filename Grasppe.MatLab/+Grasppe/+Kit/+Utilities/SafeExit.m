function SafeExit(terminateCallback, abortCallback, forceCallback, cancelCallback)
  ...
  %   file located in MatLab's search path. Simply add Grasppe.Kit.Utilities.SafeExit
  %   to enable the mechanism. Alternatively, you can specify a callback
  %   function as an argument Grasppe.Kit.Utilities.SafeExit(callback), which will
  ...
  if isempty(T) || toc(T)>3
    ...
    isQuitting(true);
    Grasppe.Kit.Utilities.DelayedCall(@(s, e)abort(abortCallback),4,'start');
    pause(0.1);
