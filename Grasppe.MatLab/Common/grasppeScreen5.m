function [ Output ] = grasppeScreen5( imagePath, ppi, spi, lpi, angle, printing, outputFolder)
...
function curve = createToneCurve(spi, lpi, angle, gain, noise, blur, radius) % screenID)
  ...
  if (exist(curvePath, 'file')==2)
    ...
    try regenerateCurve   = curveInfo.datenum < mInfo.datenum, end
  end
    ...
    for m=1:imageChannels
      ...
      for c = 1:tilesX
        for r = 1:tilesY
          ...
          progressUpdate(r+(c-1)*tilesY, 0, tiles*1.25);
          ...
          progressUpdate(r+(c-1)*tilesY, 0.5, tiles*1.25);
          ...
          progressUpdate(r+(c-1)*tilesY, 1.0, tiles*1.25);
      ...
      progressUpdate(r+(c-1)*tilesY*1.5, 1.0, tiles*1.25);
