...
%   catch err
%     recycleTimers.(space) = Grasppe.Kit.Utilities.DelayedCall(@(s, e) recycleSpace(space), 5, 'hold');
%   end
%   
%   try
...
function queueRecycleSpace(space)
  ...
  try recycleTimers.(space) = Grasppe.Kit.Utilities.DelayedCall(@(s, e) recycleSpace(space), 60*2, 'start');
